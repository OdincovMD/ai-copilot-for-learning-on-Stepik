...
function getModeInstruction(mode: LearningMode): string {
  switch (mode) {
    case "explain":
      return (
        "Режим EXPLAIN. Объясни текущий шаг простым русским языком: что проверяется, какие идеи нужны, почему это важно и где обычно путаются. "
        + "Не превращай ответ в набор подсказок к выбору варианта и не делай конспект; приоритет — понимание причинно-следственных связей."
      );
    case "hint":
      return (
        "Режим HINT. Дай обучающие подсказки без готового решения: вопросы, проверки, ограничения и направление рассуждения. "
        + "Не объясняй всю тему за пользователя и не делай конспект; приоритет — помочь самому дойти до ответа."
      );
    case "notes":
      return (
        "Режим NOTES. Подготовь краткий учебный конспект: структура шага, термины, формулы/правила, важные предупреждения и связь с контекстом. "
...
function getModeSummaryContract(mode: LearningMode): string {
  switch (mode) {
    case "explain":
      return "2-4 предложения: объясни смысл шага, проверяемые идеи и причинную связь без готового ответа.";
    case "hint":
      return "1-2 предложения: задай направление размышления и границы задачи, не раскрывая итоговый ответ.";
    case "notes":
      return "1-2 предложения: дай заголовочную выжимку конспекта, что стоит сохранить в памяти.";
  }
}
...
function getModeFocusPointsContract(mode: LearningMode): string[] {
  switch (mode) {
    case "explain":
      return [
        "3-5 пунктов с объяснением понятий, предпосылок и типичных ошибок.",
        "Каждый пункт должен отвечать на вопрос 'почему это важно для понимания?'.",
      ];
    case "hint":
      return [
        "3-5 пунктов-подсказок в форме вопросов, проверок или направлений рассуждения.",
