...
function buildFocusPoints(request: LearningRequest): string[] {
  ...
  if (taskKind === "code") {
    return [
      "Разложить задачу на входные данные, преобразование и ожидаемый результат.",
      "Продумать крайние случаи и минимальные тесты до написания финального кода.",
      ...basePoints.slice(1),
    ];
  }
  ...
  if (taskKind === "text") {
    return [
      "Сформулировать критерии полного ответа до подбора конкретных фраз.",
      "Проверить, хватает ли в ответе причины, примера и вывода.",
      ...basePoints.slice(1),
    ];
  }
  ...
  if (taskKind === "video") {
    return [
      "Опираюсь только на видимый текст страницы и посещенный контекст, если содержимое видео недоступно.",
      "Выделить термины и вопросы, которые стоит проверить после просмотра.",
      ...basePoints.slice(1),
    ];
  }
