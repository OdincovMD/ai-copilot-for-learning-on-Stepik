...
- Блок `Ответ Copilot` отправляет запрос в локальный FastAPI backend и
  показывает structured-ответ выбранного provider.
- Перед отправкой анализа сайдбар показывает короткий disclosure о данных,
  которые уйдут в запрос.
- После ответа пользователь может оценить результат; оценки сохраняются
  локально. Кнопка `Скопировать историю оценок` копирует feedback log в JSON
  для ручного beta eval, а `Очистить историю оценок` очищает только этот log.
